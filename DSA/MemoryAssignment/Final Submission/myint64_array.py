...
import ctypes
from ctypes import *
## myint64_array implementation.

class myint64_array:

    # implement the init
    # self.mymemobj = mymem.getmymemobj()
    # self.p = allocate the right space with type as 'myint64_array'
    # see myclass mem_alloc function
    # self.len = size of the myint64_array (no. of elements)

    def __init__(self, size, init=0):
        self.mymemobj = mymem.getmymemobj()
        self.len=size
        self.size=size*8
        self.p=self.mymemobj.mem_alloc(size,'myint64_array')
        ...
        self.n=-1
        for i in range(size):
            ptr=self.mymemobj.store_int64(init,hex(int(self.p,16)+(i*8)))
...
# goes out of bound.
    ...
    def __getitem__(self, i):
    	if i>self.len:
    		...
    		i=i*8
    		print(self.mymemobj.load_int64(hex(int(self.p,16)+(i))))
...
# goes out of bound.
    ...
    def __setitem__(self, i, value):
    	if i>self.len:
        	...
        	ptr=self.mymemobj.store_int64(value,hex(int(self.p,16)+(i*8)))
...
# are iterated upon.
    ...
    def __next__(self):
        if self.n<self.len-1:
            self.n+=1
            return self.mymemobj.load_int64(hex(int(self.p,16)+(8*self.n)))
        else:
            raise StopIteration
# get the property "s" 
...
# right size etc.
    ...
    def __del__(self):
        self.mymemobj.mem_free(self.p,self.len,'myint64_array')
