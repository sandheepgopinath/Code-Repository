...
class myserver():
    ...
    def __init__(self,sc, stats):
        ...
        self.stats['sidle'] += 1
        self.stats['stotal'] += 1
        self.provision_list=mylist('list')
...
#             myvm.set_idle(self) # make all the servers idle in crm
    ...
    def deprovision_vm(self, vm):
        for virtual_machine in self.provision_list:
            if virtual_machine==vm:
                print(vm.name)
                virtual_machine.state='deleted'
                bound_server=virtual_machine.sr
