...
class mycrm ():
    ...
    def __init__(self):
        ...
        self.unprovisioned=mylist('stack')
        self.idle_vm_list=mylist('stack')
            ...
            if server.sc.name==sc.name:
                if server.state=='deleted':
                    ...
                    for ser in self.server_list:
                        if ser!=server:
                            tempList.add(ser)
                    self.server_list=tempList
                else:
                    duplicate_exist_flag=1
                    raise Exception('Server name exist')
        if duplicate_exist_flag==0:
            ser=myserver(sc,self.stats)
            self.server=ser
            ...
            if self.unprovisioned.size>0:
                for i in range(self.unprovisioned.size):
                    ...
                    unprov_vm.sr=ser
                    self.provision_list.enqueue(unprov_vm)
                    ser.vl.add(unprov_vm)
                    ser.amem-=unprov_vm.amem
                    ser.acpu-=unprov_vm.acpu
                    ser.nvms+=1
    ...
    def delete_server(self, name):
        ...
        for server in self.server_list:
            if server.name==name:
                lookup_result=server 
        if lookup_result==None:
                    raise server_does_not_exist(name)
        else:
            if (lookup_result.state=='fail') |(lookup_result.state=='idle'):
                
#                 if lookup_result.state=='idle':
#                     self.stats['sfail']-=1
                if lookup_result.state=='fail':
                    self.stats['sfail']-=1
                elif lookup_result.state=='idle':
                    self.stats['sidle']-=1

                self.stats['sdeleted']+=1   
                self.stats['stotal']-=1
                lookup_result.state='deleted'
        ...
        for server in self.server_list:
            if server.name==name:
                ...
                for virtual_machine in server.vl:
                    if virtual_machine.state=='deleted':
                        pass
                    else:
                        if virtual_machine.state=='idle':
                            pass
                        elif virtual_machine.state=='active':
                            virtual_machine.stats['vidle']+=1
                            virtual_machine.stats['vactive']-=1
                        elif virtual_machine.state=='provision':
                            virtual_machine.stats['vprovision']+=1
                            virtual_machine.stats['vactive']-=1
    ...
    def provision_vm(self, name, ncpu, mem):
        ...
        for server in self.server_list:
            if server.state!='deleted':
                if server.can_provision(vc)!=None:
                    vm=myvm(vc,server,self.stats) # Provision and Total increases by 1
                    provision_status=server.provision_vm(vc,vm)
    ...
    def commit_provisioned_vms(self):
        for v_machine in self.provision_list:
            if v_machine.state=='provision':
                bound_server=v_machine.sr
                bound_server.commit_vm(v_machine)
#         for server in self.server_list:
...
#                      server.commit_vm(vms)
    ...
    def provision_undo(self, num):
        # Creating a temporary list of vm's in provision state
        ...
        if number_of_provisions==0:
            ...
            for i in range(num):
                ...
                print(vm_to_pop.name,vm_to_pop.acpu,vm_to_pop.amem,vm_to_pop.sr.name)
                bound_server=vm_to_pop.sr
                bound_server.deprovision_vm(vm_to_pop)
                if bound_server.nvms==0:
                    bound_server.state='idle'
        ...
        for server in self.server_list:
            for virtual_machine in server.vl:
                if virtual_machine.name==name:
                    server_found=1
                    virtual_machine.sr=None
                    server.amem+=virtual_machine.amem
                    server.acpu+=virtual_machine.acpu
                    server.nvms+=1
                    server.stats['vdeleted']+=1
                    server.stats['vtotal']-=1
                    if virtual_machine.state=='idle':
                        server.stats['vidle']-=1
                    elif virtual_machine.state=='provision':
                        server.stats['vprovision']-=1
                    elif virtual_machine.state=='active':
                        server.stats['vactive']-=1
#                     virtual_machine.delete(server)
                    virtual_machine.state='deleted'
                    break
    ...
    def provision_idle_vms(self, sr):
        ...
        for virtual_machine in sr.vl:
            if virtual_machine.state=='idle':
                total_idle_vms+=1
                vm_provision_status=0
                for server in self.server_list:
                    if server.provision_vm(virtual_machine.vc,virtual_machine)==virtual_machine:
                        print('Provisioned',virtual_machine.name)
                        self.stats['vprovision']+=1
                        self.stats['vidle']-=1
                        virtual_machine.sr=sr
                        provisioned_vms+=1
                        vm_provision_status=1
                        self.provision_list.enqueue(virtual_machine)
                if vm_provision_status==0:
                    print('Unable to provision ',virtual_machine.name,virtual_machine.amem,virtual_machine.acpu)
                    self.unprovisioned.push(virtual_machine)
#             else:
#                 print('Not in idle state',virtual_machine.name,virtual_machine.amem,virtual_machine.acpu)
    ...
    def provision_all_idle_vms(self):
        ...
        for server in self.server_list:
            for virtual_machine in server.vl:
                if virtual_machine.state=='idle':
                    self.idle_vm_list.push(virtual_machine)
        for i in range(self.idle_vm_list.size):
            virtual_machine=self.idle_vm_list.pop()
            provision_status=0
            for server in self.server_list:
                ...
                vm=server.provision_vm(virtual_machine.vc,virtual_machine)
                
#                
                
                if vm!=None:
                    print(virtual_machine.vc)
